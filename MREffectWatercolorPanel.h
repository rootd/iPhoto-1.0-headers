/*
 *     Generated by class-dump 3.1.2.
 *
 *     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2007 by Steve Nygard.
 */

#import "MREffect.h"

@class MRCroppingSprite, MRImage, MRTextRenderer, NSMutableDictionary;

@interface MREffectWatercolorPanel : MREffect
{
    NSMutableDictionary *mSprites;
    MRTextRenderer *mTextRenderer;
    MRImage *mTextImage;
    MRCroppingSprite *mTextSprite;
}

- (int)_maxLinesForTextElement:(id)fp8;
- (struct CGSize)_maxSizeForTextElement:(id)fp8;
- (BOOL)getVerticesCoordinates:(struct CGPoint (*)[4])fp8 withMatrix:(float [16])fp12 forElement:(id)fp16;
- (id)elementHitAtPoint:(struct CGPoint)fp8 withInverseMatrix:(float [16])fp16 localPoint:(struct CGPoint *)fp20;
- (void)renderEffectSubtitleAtTime:(double)fp8 inContext:(id)fp16 withArguments:(id)fp20;
- (void)renderEffect3AtTime:(double)fp8 inContext:(id)fp16 withArguments:(id)fp20;
- (void)renderEffect2AtTime:(double)fp8 isSquare:(BOOL)fp16 inContext:(id)fp20 withArguments:(id)fp24;
- (void)renderAtTime:(double)fp8 inContext:(id)fp16 withArguments:(id)fp20;
- (BOOL)isLoadedForTime:(double)fp8;
- (void)_cleanup;
- (id)initWithEffectID:(id)fp8;

@end

